...
json_dataset = load_dataset(path='json',
                           data_files=data_dir,
                           split='train')
dataset=json_dataset.train_test_split(test_size=0.1)
print(dataset)

def collate_fn(data):
    # text_pairs = [(i['src'], i['tgt']) for i in data]
    seq=[i['src_text'] for i in data]
    target=[i['tgt_text'] for i in data]
    #编码
...
def train(epochs,sign=0):
    if sign==1:
        loaded_model = torch.load('model/智能问答.model')

        # 将模型移到训练设备（例如 GPU）
        loaded_model = loaded_model.to('cuda')
        model=loaded_model
    else:
        model = GPT2LMHeadModel.from_pretrained("uer/gpt2-distil-chinese-cluecorpussmall").to(device)
    ...
    for epoch in range(epochs):
        ...
        sched.step()
        torch.save(model, 'model/智能问答.model')
